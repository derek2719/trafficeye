...
var paramDemo = '{"area":[{"city":"宁波","city_code":"101210401","route_id":"0574_106_001","route_name":"高桥-望春"}],"width":400,"height":460,"url":"http://mobile.trafficeye.com.cn:8000/api2/v2/pics/","density":"2.0"}';
...
function initHtml(data, deviceInfo) {
	...
	var urlSuffix = "?width=" + Math.floor(imageW * density) + "&height=" + Math.floor(imageH * density);
	for (var k in data) {
		var key = k;
		var area = data[k];
		area.tabIndex = index;
		...
		if (area) {
			...
			if (shareElem) {
				...
				shareElem.addEventListener("touchend", function(evt) {
					...
					window.share.shareMethod(areas[areaKey].city + " " + areas[areaKey].routeName);
...
$(function() {
	...
	function touchend_back(e) {
		...
		window.initTransfrom.gotoPrePage();
	}//返回
	initByParam(window.initTransfrom.initmethod());
	...
	setElem.addEventListener("touchstart", function(evt) {
		...
		window.initTransfrom.goFavorite();
	});
