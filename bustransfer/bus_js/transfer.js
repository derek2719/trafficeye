(function() {
	...
	function createLiHtml(data) {
		...
		for (var i = 0, len = data.length; i < len; i++) {
			htmls.push("<li onclick='openlihandler(this);'>");
			htmls.push("<a class='a" + (i-0+1) + "''>");
			var details = [];
			...
			htmls.push("<h2>"+temp+"</h2>");
			htmls.push("<p>换乘" + lineInfo.length + "次全程 约" + data[i].brldist + "公里 约" + data[i].brltime + "分钟</p>");
			htmls.push("<span class='jtd'></span>");
			htmls.push("</a>");
	...
	var osStartKey = os.get("busstart");
	var osEndKey = os.get("busend");

	var startObj = osStartKey.split(",");
	var endObj = osEndKey.split(",");
	...
	function setStartEndName(sName, eName) {
		startNameElemZ.html(sName);
		endNameElemZ.html(eName);
		showResultHandler.clean();
		showResultHandler.transferQuery(tabLiIndex);
	};
	setStartEndName(startObj[0], endObj[0]);
	...
	window.openlihandler = function (evt) {
		var liElemZ = $(evt);
