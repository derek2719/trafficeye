...
for (var i = 0; i < res.length; i++) {
	...
	if ($.trim(res[i].city) == "深圳") {
		...
		heplpage = "shenzhen";
				...
				str = "上海市综合交通研究中心（www.hzjtydzs.com）"
				heplpage = "shanghai";
			};;
...
//	htmls.push("<li class='ia'id='share'><a href=\"javascript:void(0);\"><span></span></a></li>");
//	htmls.push("<li class='ic'id='help'><a href=\"index_help_" + helppage + ".html\"  onclick=\"window.localStorage.pre='index_list.html';\"><span></span></a></li>");
//	htmls.push("</ul></div></div>");
		...
		var htmls1 = [];
			...
			$.each(result.trafficIndexList, function(i, item) {
				...
				if (item.degree == "畅通") {
					...
					textcolor = "black";
					bgcolor = "#FF6600";
				} else if (item.degree == "严重拥堵"||item.degree == "拥堵") {
					bgcolor = "#CC0000";
				} else if (item.degree == "拥堵"||item.degree == "拥挤") {
					bgcolor = "#CC0000";
				...
				htmls1.push("<div  style='background: " + bgcolor + "; opacity: .60;display:block ;margin:0px;padding:0px;border-bottom:1px #ccc solid;border-left:1px #ccc solid;border-right:1px #ccc solid;font-size: 13px ;height:24px'>	<span style='text-align:center;float:left;margin-left: 8px;width:140px;color:" + textcolor + "'>" + item.area + " </span><span  style='margin-left: 10px;color:" + textcolor + ";'> " + item.index + " </span><span style='margin-left: 20px;color:" + textcolor + ";'>  " + item.degree + "</span><span style='float:right;margin-right:15px;color:" + textcolor + ";'> " + item.speed + "km/h </span></div>");
           ...
           htmls1.push("<script type='text/javascript'>	" + "$(function() {document.getElementById('share').addEventListener('touchstart', function() {$('#share').attr('src','images/icon_share_pressed.png');});" + "document.getElementById('share').addEventListener('touchend', function() {" + "share($('#city').text()+' '+$('#area').text()); $('#share').attr('src','images/icon_share.png');});});");
			...
			$("#listRes").html(html);
			$("#time").html(time);
			$("#datasource").html("数据来源：" + str + "发布的公开数据");
			$("#waiting").hide();
...
function share(shareText){
    ...
    //                    alert(shareText);
    //                    alert(shareTextEncode1);
    //                    alert(shareTextEncode2);
    //                    shareTextEncode = encodeURI(shareText);
