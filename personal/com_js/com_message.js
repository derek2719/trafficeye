...
(function(window) { //闭包
        ...
        creatMessagesListHtml : function(data) {
            ...
            if(data.messageType == "event"){
                if(data.media_small){//显示事件缩略图
                    htmls.push("<li><div class='r-img' onclick=\"gotoEvent('"+data.uid+"','"+data.message_id+"',this);\"><img src='" + data.media_small + "' alt='' width='40' height='40' /></div>");
                }
                htmls.push("<li><div class='h-img' onclick=\"gotoEvent('"+data.uid+"','"+data.message_id+"',this);\"><img src='" + avaterSrc + "' alt='' width='40' height='40' /></div>");
                htmls.push("<div class='h-text xt' onclick=\"gotoEvent('"+data.uid+"','"+data.message_id+"',this);\"><h3>" + data.username + "</h3>"+data.message+"</div>");
            }else if(data.messageType == "message"){
                htmls.push("<li><div class='h-img' onclick=\"gotoChat("+data.uid+",this);\"><img src='" + avaterSrc + "' alt='' width='40' height='40' /></div>");
                htmls.push("<div class='h-text xt' onclick=\"gotoChat("+data.uid+",this);\"><h3>" + data.username + "</h3>"+data.message+"</div>");
            }else if(data.messageType == "fans"){
                htmls.push("<li><div class='h-img' onclick=\"gotoLookfans(this);\"><img src='" + avaterSrc + "' alt='' width='40' height='40' /></div>");
                htmls.push("<div class='h-text xt' onclick=\"gotoLookfans(this);\"><h3>" + data.username + "</h3>"+data.message+"</div>");
             }
    ...
    UserLetters.prototype = {//方法挂载在原型链
        ...
        mergeNewUserLettersData : function() {
         ...
         creatLettersHtml : function(data) {
            ...
            htmls.push("<div class='h-text xt' onclick=\"gotoChat("+data.friend_id+",this);\"><h3>" + data.username + "</h3>"+data.time+"前</div>");
            htmls.push("</li>");
            return htmls.join("");
        },
    ...
    PageManager.prototype = {
        ...
        gotoChat : function(friend_id,evt) {  
            ...
            setTimeout((function(){
                $(elem).removeClass("curr");  
                Trafficeye.toPage("com_chat_message.html");
        ...
        lookAllCommentbtnUp : function(uid,publishId,evt) {
            ...
            setTimeout((function(){
                $(elem).removeClass("curr");  
                Trafficeye.toPage("com_personaldetails.html");
            }),Trafficeye.MaskTimeOut);
        },
        /**
         * 进入首页显示未读消息或私信数字
